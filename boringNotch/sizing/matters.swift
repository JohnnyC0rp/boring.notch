...
let expandedUtilitySize: CGSize = .init(width: 640, height: 310)
let windowSize: CGSize = .init(width: openNotchSize.width, height: expandedUtilitySize.height + shadowPadding)

func notchOpenSize(for view: NotchViews) -> CGSize {
    view == .calendar || view == .clipboard ? expandedUtilitySize : openNotchSize
}
