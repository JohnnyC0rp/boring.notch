...
enum CalendarTimelineGeometryTests {
    static func main() {
        ...
        require(byID["midnight"]!.x == 0 && byID["midnight"]!.width == 92, "Overnight event must clip to this day")
        require(byID["long"]!.width == 184, "Duration must determine exact block width")
        require(byID["long"]!.lane != byID["overlap"]!.lane, "Overlaps must use separate lanes")
        require(byID["adjacent"]!.lane == 0, "Finished lanes must be reused")
        require(abs(byID["short"]!.width - 92 / 60) < 0.0001 && byID["short"]!.hitWidth == 24,
                "Short event needs exact visible duration and a larger invisible hit target")
        require(byID["short"]!.lane != byID["short-neighbor"]!.lane, "Short event hit targets must not collide")
        require(byID["instant"]!.width == 0 && byID["instant"]!.hitWidth == 24, "Instant event must remain selectable")
        ...
        require(compactByID["long"]!.width == 108, "Compact blocks must use the compact hour scale")
        require(abs(compactByID["short"]!.width - 0.9) < 0.0001 && compactByID["short"]!.hitWidth == 24,
                "Compact short events retain full pointer targets without inflated durations")
        require(compactByID["short"]!.lane != compactByID["short-neighbor"]!.lane,
